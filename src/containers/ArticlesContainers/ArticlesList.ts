...
import { addArticleToFavorite, removeArticleFromFavorite } from '../../store/reducers/user/userUtils';
...
const dispatchToProps = (dispatch: ThunkDispatch<{}, {}, any>) => ({
	favorite: (token: string) => (articleSlug: string) => dispatch(addArticleToFavorite(articleSlug, token)),
	unFavorite: (token: string) => (articleSlug: string) => dispatch(removeArticleFromFavorite(articleSlug, token)),
